...
import { listProjects, createProject, getFlowsheet, saveFlowsheet, updateProject, } from '../lib/api-client';
let saveTimeout = null;
function debounceSave(get, set) {
    if (saveTimeout)
    ...
    saveTimeout = setTimeout(() => {
        ...
        if (currentProjectId) {
            ...
            saveFlowsheet(currentProjectId, nodes, edges, simulationBasis)
                .then(() => set?.({ saveStatus: 'saved' }))
                .catch(() => set?.({ saveStatus: 'error' }));
        }
    }, 1000);
}
export const useFlowsheetStore = create((set, get) => ({
    nodes: [],
    edges: [],
    selectedNodeId: null,
    selectedEdgeId: null,
            ...
            if (projects.length > 0) {
                ...
                try {
                    ...
                    if (flowsheet.simulation_basis) {
                        const basis = flowsheet.simulation_basis;
                        set({
                            simulationBasis: {
                    ...
                    if (flowsheet.nodes && flowsheet.nodes.length > 0) {
                        const equipment = flowsheet.nodes.map((n) => ({
                            id: n.id,
                        ...
                        const streams = (flowsheet.edges ?? []).map((e) => ({
                            id: e.id,
                            sourceId: e.source,
