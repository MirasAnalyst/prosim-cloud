...
import { agentChat, getChatHistory, saveChatMessages, clearChatHistory } from '../lib/api-client';
import { getDefaultParameters } from '../lib/equipment-library';
import { autoLayout } from '../lib/auto-layout';
import { useFlowsheetStore } from './flowsheetStore';
import { useSimulationStore } from './simulationStore';
const VALID_TYPES = new Set(Object.values(EquipmentType));
function applyFlowsheetAction(action) {
    ...
    const validConnections = action.connections.filter((c) => validIds.has(c.source_id) && validIds.has(c.target_id));
    // Compute positions via auto-layout
    const positions = autoLayout(validEquipment.map((eq) => eq.id), validConnections);
    const posMap = new Map(positions.map((p) => [p.id, { x: p.x, y: p.y }]));
    // Build equipment array
    const equipment = validEquipment.map((eq) => {
...
export const useAgentStore = create((set, get) => ({
            ...
            if (fsNodes.length > 0) {
                ...
                if (simResults) {
                    flowsheetContext.simulationResults = {
                        ...
                        logs: simResults.logs?.filter((l) => l.startsWith('WARNING') || l.startsWith('ERROR')).slice(0, 20),
                    };
                }
            }
            const data = await agentChat(chatMessages, Object.keys(flowsheetContext).length > 0 ? flowsheetContext : undefined);
    ...
    clearMessages: () => {
        ...
        const projectId = useFlowsheetStore.getState().currentProjectId;
        if (projectId) {
            clearChatHistory(projectId).catch((err) => console.error('Failed to clear chat history:', err));
        }
    },
