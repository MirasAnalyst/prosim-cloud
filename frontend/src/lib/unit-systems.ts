...
export type UnitSystemName = 'SI' | 'Field' | 'CGS';
...
export interface UnitLabels {
  ...
  massFlow: string;
  molarFlow: string;
  enthalpy: string;  // per mass
  entropy: string;  // per mass
  density: string;
  viscosity: string;
  thermalConductivity: string;
  surfaceTension: string;
  heatCapacity: string;  // Cp, Cv per mass
  power: string;  // duty, work
...
export interface UnitSystem {
  ...
  label: string;
  units: UnitLabels;
  // Conversion functions FROM display units TO SI
  toSI: {
    temperature: (v: number) => number;
    pressure: (v: number) => number;
    massFlow: (v: number) => number;
    power: (v: number) => number;
  };
  // Conversion functions FROM SI TO display units
  fromSI: {
    temperature: (v: number) => number;      // °C → display
    temperatureDelta: (v: number) => number; // ΔK/Δ°C → display (no offset, only scale)
    pressure: (v: number) => number;         // kPa → display
    massFlow: (v: number) => number;         // kg/s → display
    molarFlow: (v: number) => number;        // mol/s → display
    enthalpy: (v: number) => number;         // kJ/kg → display
    entropy: (v: number) => number;          // kJ/(kg·K) → display
    density: (v: number) => number;          // kg/m³ → display
    viscosity: (v: number) => number;        // Pa·s → display
    thermalConductivity: (v: number) => number;  // W/(m·K) → display
    surfaceTension: (v: number) => number;   // N/m → display
    heatCapacity: (v: number) => number;     // J/(kg·K) → display
    power: (v: number) => number;            // kW → display
    volumetricFlow: (v: number) => number;   // m³/s → display
    velocity: (v: number) => number;         // m/s → display
    area: (v: number) => number;             // m² → display
    length: (v: number) => number;           // m → display
  };
}
...
export const UNIT_SYSTEMS: Record<UnitSystemName, UnitSystem> = {
  SI: SI_SYSTEM,
  Field: FIELD_SYSTEM,
  CGS: CGS_SYSTEM,
};

export function getUnitSystem(name: UnitSystemName): UnitSystem {
 ...
 */
export function formatValue(value: number | null | undefined, type: keyof UnitLabels, unitSystem: UnitSystem): string {
...
function convertFromSI(value: number, type: string, unitSystem: UnitSystem): number | null {
  const fn = (unitSystem.fromSI as Record<string, (v: number) => number>)[type];
  if (!fn) return value;
  try {
    return fn(value);
  } catch {
